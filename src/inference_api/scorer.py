...
def _load_model(stat_type: str):
    ...
    if filename is None:
        _cache[stat_type] = None; return None
...
def _build_features(req: PredictRequest) -> list[float]:
    ...
    return [req.rolling_5_avg, req.rolling_10_avg, req.rolling_5_std,
...
def score(req: PredictRequest) -> PredictResponse:
    ...
    if   probability >= 0.55: recommendation = "OVER"
    elif probability <= 0.45: recommendation = "UNDER"
    else:                      recommendation = "NO_EDGE"

    return PredictResponse(probability=round(probability,4), confidence=confidence,
                           recommendation=recommendation, model_version=__model_version__,
                           source=source)
