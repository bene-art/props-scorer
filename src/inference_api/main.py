...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "service_startup",
        extra={"version": __version__, "model_version": __model_version__},
    )
    yield
...
async def predict(request: Request, payload: PredictRequest):
    ...
    logger.info(
        "prediction_complete",
        extra={"request_id": rid, "probability": result.probability, "source": result.source},
    )
    return result
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(
        "unhandled_exception",
        extra={"request_id": rid, "error_type": type(exc).__name__, "error_message": str(exc)},
    )
    return JSONResponse(
        status_code=500,
